...
export class BchatProtocolEncryption implements EncryptionProvider {
  ...
  async decryptEnvelope(
    ...
    return {
      ...
      previews: dataMessage?.previews,
      openGroupInvitation: dataMessage?.openGroupInvitation,
      payment: dataMessage?.payment,
      sharedContact: dataMessage?.sharedContact,
      expireTimer: dataMessage?.expireTimer,
      isExpirationTimerUpdate: dataMessage?.isExpirationTimerUpdate,
      syncTarget: dataMessage?.syncTarget,
      hasGroupContext: dataMessage?.hasGroupContext,
      typing: content?.typing,
      receipt: content?.receipt,
      unsend: content?.unsend,
      dataExtraction: content?.dataExtraction,
      messageRequestResponse: content?.messageRequestResponse,
      call: content?.call,
      senderBchatId,
